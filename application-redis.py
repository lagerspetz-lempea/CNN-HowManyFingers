...
def sendImage(inframe, outframe, url, labels):
    # Base64 encode input image
    img = cv.imencode('.jpg', inframe)[1]
    img_b64 = base64.b64encode(img).decode('utf-8')

    outimg = cv.imencode('.jpg', outframe)[1]
    outimg_b64 = base64.b64encode(outimg).decode('utf-8')

    timestamp = datetime.datetime.now().isoformat()

    # Build message to save in the redis db.
    redis_msg = {
        'stream_id': url,
        'timestamp': timestamp,
        'objects': labels,
        'img_input': img_b64,
        'img_output': outimg_b64
    }
...
def main():
    ...
    while True:
        ...
        # Only one "object" (the hand) with one label, the finger count
        labels = [pred]
        # Send data over
        sendImage(frame, window, url, labels)
